...
## Overview
This project includes seeders for testing the chat application with sample data.

### Seeders:
- **UserSeeder**: Creates 6 test users (1 admin + 5 regular users)
- **ChatSeeder**: Creates 5 chats with realistic messages (3 personal + 2 group chats)
- **DatabaseSeeder**: Runs both seeders in order
...
### Users
- **Admin**: youremail@email.com / Developer
- **Users**: alice@example.com, bob@example.com, charlie@example.com, diana@example.com, edward@example.com (all use password: `password`)

### Chats
- 3 Personal chats (Developer ↔ Alice, Bob, Charlie)
- 2 Group chats (Project Team Alpha, Laravel Study Group)
- Multiple timestamped messages in each chat

## Notes
- ChatSeeder depends on UserSeeder (run users first if seeding individually)
- All users are email-verified and ready to use
- Messages have realistic timestamps for testing
...
class ChatSeeder extends Seeder
{
    ...
    {
        // Get users
        $main = User::where('email', 'youremail@email.com')->first();
