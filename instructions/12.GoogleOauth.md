...
### 2. **Secure Token Flow**
1. Google OAuth → Backend creates 1-minute token → URL redirect
2. Frontend catches token → Calls `patchRefreshToken(token)` API
3. Backend validates short token → Returns new long-lasting token
