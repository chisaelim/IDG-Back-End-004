...
```
IDG-Back-End-004/
├── Dockerfile                          # Updated: Add permissions
├── docker-compose.prod.yml             # New: Backend production config
├── production.sh                       # New: Backend startup script
└── frontend/
    ├── Dockerfile                      # Updated: Add build stage
    └── docker-compose.prod.yml         # New: Frontend production config
```
...
services:
  app:
    build: .
    image: ultralink.cloud:latest
    container_name: ultralink.cloud-container
    volumes:
    ...
    environment:
      ...
      APP_ENV: production
      APP_DEBUG: false
      APP_URL: https://ultralink.cloud
      APP_FRONTEND_URL: https://ultralink.cloud
      APP_EMAIL_VERIFICATION_URL: "https://ultralink.cloud/email/verify"
      APP_PASSWORD_RESET_URL: "https://ultralink.cloud/password/reset"

      # Google OAuth (CRITICAL)
      GOOGLE_CLIENT_ID: ${GOOGLE_CLIENT_ID}
      GOOGLE_CLIENT_SECRET: ${GOOGLE_CLIENT_SECRET}
      GOOGLE_REDIRECT_URI: "https://ultralink.cloud/api/auth/google/callback"
  ...
  mysql:
    ...
    container_name: mysql-ultralink.cloud-container
    environment:
  ...
  phpmyadmin:
    ...
    container_name: phpmyadmin-ultralink.cloud-container
    depends_on:
      - mysql
    environment:
      UPLOAD_LIMIT: 50M
...
volumes:
  mysql:
    name: ultralink.cloud-mysql
  node:
    name: ultralink.cloud-node
  laravel:
    name: ultralink.cloud-laravel
```

**Key changes from development:**
- Uses `production.sh` instead of `start.sh`
- APP_ENV=production, APP_DEBUG=false
- HTTPS URLs (https://ultralink.cloud)
...
RUN npm install
COPY . .
ENV VITE_API_URL=https://ultralink.cloud/api
RUN npm run build
...
Your production Docker setup is ready! 🚀
